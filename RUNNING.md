...
| `FF_NO_RHW_ALPHAMOD_FIX=1` | Reverts **the GMRADAR-4 fix**: the GL layer's chroma-key repair forced fragment alpha to come from the texture on every alpha-tested 2D textured draw, discarding the vertex alpha of draws whose D3D ALPHAOP is MODULATE. The GM radar noise overlay (vertex alpha 0.3) drew at alpha 1.0 — measured green mean 36/255 instead of 11 — and buried the returns. With the fix the scope background is near-black like the Windows build and the building blips read. Only set this to reproduce the washed-out scope. |
| `FF_NO_GNDMOVE_DEST_FIX=1` | Reverts **the GNDMOVE-1 fix**: a Tactical Engagement's ground battalions load from the mission file with `dest_x/dest_y == 0` and an objective, but nothing ever turns that objective into a destination — `PickFinalLocation()` is only reached through `SetUnitOrders()`, which short-circuits when the orders and objective already match what the file set. `GetUnitDestination()` then returns `(-1,-1)`, every grid path to it fails, and the column never moves for the whole mission while still reporting 40 ft/s to the campaign. With this flag set, the Maverick TE's tank column is frozen and **GMT is blank**; without it the tanks drive at ~60 ft/s and GMT paints them. Only set this to reproduce the stationary column. |
| `FF_NO_GMT_DARK_SCOPE=1` | Reverts **the GMRADAR-6 fix**: the GM composite draws the terrain map unconditionally (`gmcomposit.cpp`, `case Ground`) because it has no notion of the radar mode, so GM/GMT/SEA painted an identical picture and the handful of sub-pixel mover blips were invisible under it (PO: "GM/GMT/SEA showed the same image exactly"). With the fix, GMT and SEA clear the sweep and draw only their movers — measured ground-map mean 3.93 → 0.00 — and movers get an intensity floor plus a 2x2 block so a column reads as a line (contact max 24 → 200, against a 10.9 noise floor). Set this to get a terrain map in every mode again. |
| `FF_GM_MOVER_INTENSITY=<0-255>` | The GMT/SEA mover blip intensity floor (default 200). `0` disables the floor and returns movers to their computed `r * 32 * gain * GainScale`, which is ~24/255 at 40 nm. |
| `FF_NO_TE_ROVING_ARMOR=1` | Reverts **GNDMOVE-2** (PO-directed, *not* a port fix): in a Tactical Engagement a battalion that reaches its destination is sent back to its objective, and from the objective out to a reachable neighbouring cell, so it patrols that leg for the whole mission instead of arriving and parking. Without it the Maverick TE's column drives for ~74 s and then sits. Campaign ground movement is untouched either way. |
...
| `FF_DEBUG_NVGALPHA=1` | `[NVGALPHA]` one-shot dump of the **alpha chain** across texture units 0–3 (env mode, combineRGB, combineALPHA, source0 alpha, texture enabled), triggered the first time `DOTPRODUCT3` is applied — an op only the `DX_NVG` pipeline uses. Tests whether units 0–2 carry stale alpha combines upstream of unit 3, the NVG-5 hypothesis. |
| `FF_DEBUG_RS=1`, `FF_DEBUG_TSS=1` | `[RS-UNHANDLED]` / `[TSS-UNHANDLED]` — a D3D **render state** or **texture stage state** the GL layer does not implement, and therefore silently ignores. Both switches previously used `D3DGL_LOG`, which compiles to nothing (`D3DGL_DEBUG` is 0), so these were invisible. First-line diagnostic when something renders subtly wrong: it names the state D3D asked for and GL never got. |
| `FF_TEST_GEARDOWN=<sec>` | drops the gear handle at a fixed time, so gear behaviour can be tested by script |
| `FF_DEBUG_GMPOS=1` | `[GMPOS]` where each GMT/SEA mover lands on the scope — entity id, world XY, scope centre, ground-map range, and the normalised `rx`/`ry` plus the in-cone verdict. This is how "the contacts are in the wrong place" gets told apart from "the tanks never entered the list". |
| `FF_DEBUG_GNDMOVE=1` | `[GNDMOVE]` ground-unit movement decisions: `enter` (per `BattalionClass::MoveUnit` — location, objective, orders, destination, waypoint count, current waypoint), `decodeWP` (what each unit's waypoint list and destination decode to **at load**, which is how the empty TE destination was found), `PickFinalLocation` (the destination actually chosen, or that there was no objective to choose from), `VEGITATE` (a battalion returning early with no objective), `WATER HALT`, and the per-lead GNDAI state. First-line diagnostic for "the ground units are not moving". |
| `FF_DEBUG_GM=1` | `[GM] targets` — per Targets pass: mode, contacts walked / out of range / scan-rejected / drawn / suppressed by a lock; `[GM] blips` — the sim-object blip math (radius in pixels, mean intensity, clipped, shaped). Plus the GMRADAR-2/3 pipeline lines (`xform`, `blitToTex`, `destTex`, `quadFlush`). `FF_DEBUG_GM_EVERY=1` prints every pass instead of every 20th. |
| `FF_GM_DUMP=<dir>` | Dumps the GM radar's 128×128 render target as PPM at four points of each sweep: `gm_grnd_*` after the ground map, `gm_tgts_*` after the target returns, `gm_pre_*` / `gm_post_*` before / after the noise overlay (every 4th call, 300 max). This is the instrument for anything the scope shows — the MFD in a video is three blends removed from it. `scripts/qa/gm-noise-level.sh` measures the post-noise level. |
