...
4. **#14 Missile launch in dogfight broken** (user, June 5): launching with space bar or 2nd joystick trigger was always balky and has stopped working entirely. **Gun (1st trigger) has always worked in dogfight** - so the input path itself is alive; the fault is in weapon-release/pickle specifically (DIK_SPACE pickle command and the 2nd-trigger button binding) or weapon-select/SMS state (missile selection, master arm, uncage) in dogfight spawns.
