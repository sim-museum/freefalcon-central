...
- **Tactical Engagement "09 Landing Final Approach" (and "10 Instrument Landing") crashed on COMMIT/load → now fails gracefully.** Root cause: these v73 missions' unit data overruns the decode buffer (`memcpychk` throws `InvalidBufferException`/`std::out_of_range` at the 2nd unit, type 0x0197) — a genuine unit-decode desync, NOT a version mismatch (the mission is already v73). On Windows SEH swallowed it; on Linux the uncaught throw → `std::terminate` → SIGABRT. **Fix: wrap `TheCampaign.LoadCampaign()` in `FM_LOAD_CAMPAIGN` with `try/catch(std::exception&)` → route to `FM_JOIN_FAILED` (back to menu).** CRITICAL detail: the throw unwinds past the **recursive** `campCritical` leaves in `Decode`/`LoadCampaign` (Windows `CRITICAL_SECTION` semantics, counted), leaving the main thread holding it N-deep → the campaign thread would deadlock. The catch drains it via `while (F4CheckHasCriticalSection(campCritical)) CampLeaveCriticalSection();`. Verified: landing mission → "failing load gracefully" → menu at 59 FPS, responsive, no deadlock. **~~The mission still won't LOAD~~ — SUPERSEDED, verified 2026-08-27: it loads and flies.** TESWEEP rows 09 and 10 both reach `RunningGraphics` with **zero** `InvalidBufferException` and zero `JOIN_FAILED`, and TE-09 is the mission the PO flew repeatedly through the whole TERRAIN-Z/gear investigation. So the "incompatible unit data / deep format issue" conclusion is wrong — do **not** treat these missions as broken or as off-limits for testing. The mechanism is *not* established, but the most plausible candidate is that the decode desync was never a format problem at all: `memcpychk` throwing at the 2nd unit is exactly what heap corruption from the allocator mismatches fixed this session (5243 → 0 ASAN errors, plus two heap-buffer-overflow READs) would produce. Offered as a hypothesis, not a finding. The try/catch hardening described above is still correct and still worth having. This try/catch now hardens ALL campaign/TE/dogfight loads against decode desync crashes.
