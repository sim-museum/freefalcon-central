...
9. **OPEN: radar shows no contacts in dogfight** (user report June 6): enemy aircraft never appears on the FCR in DOGFIGHT — user found it visually with Shift-6. On Wine the B-scope shows the contact. NOTE: radar DOES detect in Instant Action (HUD-view screenshot shows FCR contact at 22nm) — so the scan/detect chain works and the problem is dogfight-specific: suspects are team/IFF filtering of the dogfight opponent, the dogfight flight's entity flags (e.g. not in the sensor target list), or campaign-class differences for DF flights. Instrument src/sim/radar/radardoppler.cpp scan loop with env-gated FF_DEBUG_RADAR counting candidates vs rejects, run automated two-ship dogfight (add AI via roster right-click; RequestACSlot(player=0) traced by FF_DEBUG_DF).
