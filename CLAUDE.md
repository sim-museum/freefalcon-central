...
8. **NEW: 3-view cockpit doesn't match Windows reference** (user report June 6, dogfight session): the correct 3-view (Wine screenshot) shows the pilot-eye view with the bright fully-textured instrument panel (texture 912), MFDs and HUD. Our virtual-pit camera/view differs (FF_VIEW_SCRIPT mode-4 screenshots show the pit from above/behind, exterior visible — note mode 4 uses the Linux-added Mode3DCockpit case in otwloop.cpp which may not replicate the real '3'-key view setup; the user's in-game 3-key view needs a fresh screenshot to compare). Head position chain: vcock.cpp VCock_Display → headOrigin/headPan/Origin + SetCamera(&headOrigin,&headMatrix). Need user's current 3-view screenshot to confirm symptom.
9. **NEW: radar shows no contacts in dogfight** (user report June 6): enemy aircraft never appears on the FCR — user found it visually with Shift-6. On Wine the B-scope shows the contact. Suspects: radar scan target list empty (sensor target list / sim object lists), detection math (FloatToInt32/64-bit), or RCS/range data zeroed. Start in src/sim/radar/radardoppler.cpp scan loop — instrument with an env-gated FF_DEBUG_RADAR counting candidates vs detections during an automated two-ship dogfight (add an AI via roster right-click popup first; the add-AI path RequestACSlot(player=0) is traced by FF_DEBUG_DF).
