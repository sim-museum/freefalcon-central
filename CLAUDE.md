...
2. **#12 3-view virtual pit interior black tub** — IN PROGRESS (June 5/6 session, deeply narrowed):
   - Pit model (vrCockpit) loads, draws, and ALL 11 of its textures have valid DXT pixel data (verified via FF_DEBUG_VPIT texture/SURFSTAT dump).
   - Pixel attribution (FF_PROBE_PIXEL on a tub pixel): the black is painted by the **pit geometry itself** — `fvf=0x1d2` = D3DFVF_MANAGED (XYZ|NORMAL|DIFFUSE|SPECULAR|TEX1), pit-mode pass, texture 913 bound (valid data), 2700 indices.
   - **Working theory**: those faces' baked vertex DIFFUSE is black; on Windows their visible color comes from per-vertex EMISSIVE — DXEngine sets `D3DRENDERSTATE_EMISSIVEMATERIALSOURCE = D3DMCS_COLOR2` (emissive = vertex SPECULAR channel, dxengine.cpp DrawSurface). GL fixed-function has no per-vertex emissive; the compat layer drops it → black.
   - Changes kept (more correct regardless): pre-lit lighting-disable in d3d_gl.cpp VB draws now keyed on `DIFFUSE && !NORMAL` (terrain) instead of any-DIFFUSE; lit object draws (NORMAL present) get `GL_COLOR_MATERIAL(GL_DIFFUSE)` so vertex diffuse acts as material diffuse; pit-mode force-disable of GL lighting in dxengine.cpp now opt-in via `FF_PIT_NO_LIGHTING=1` (default = lit, matching Windows).
   - **Next step**: approximate COLOR2-emissive in d3d_gl.cpp — e.g. secondary-color array + GL_COLOR_SUM for emissive draws, or substitute the specular channel into the primary color for unlit emissive pit surfaces (the compat VBs are system-memory, so CPU-side channel swap per draw is feasible).
